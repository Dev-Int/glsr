...
class EditUser extends AbstractController
{
    ...
    {
        $userCommand = $this->serializer->deserialize($request->getContent(), EditUserCommand::class, 'json');
        $userCommand->uuid = $uuid;

        $this->commandBus->dispatch($userCommand);

        return $this->jsonResponse->response('User update started!', Response::HTTP_FOUND);
    }
}
